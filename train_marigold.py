...
# Marigold-style LoRA fine-tuning (optional import)
try:
...
if not PEFT_AVAILABLE:
    logger.warning(
        ...
        "LoRA features will be disabled. Use --use_lora flag only after installing PEFT."
    )
...
def compute_enhanced_features(noise, conf):
    ...
    Adds gradient and magnitude information to improve edge and structure awareness.
    ...
    Returns:
        Enhanced features [B, 6, H, W] including:
        - noise (原始噪声)
        - conf (原始置信度)
        - noise_dx (噪声x方向梯度)
        - noise_dy (噪声y方向梯度)
        ...
        - conf_dy (置信度y方向梯度)
    """
    # 计算噪声的梯度 (使用Sobel算子获得更好的边缘检测)
    noise_dx = noise[:, :, :, :-1] - noise[:, :, :, 1:]
    noise_dx = F.pad(noise_dx, (0, 1, 0, 0), mode='replicate')  # 恢复原始尺寸

    noise_dy = noise[:, :, :-1, :] - noise[:, :, 1:, :]
    noise_dy = F.pad(noise_dy, (0, 0, 0, 1), mode='replicate')  # 恢复原始尺寸

    # 计算置信度的梯度
    conf_dx = conf[:, :, :, :-1] - conf[:, :, :, 1:]
    conf_dx = F.pad(conf_dx, (0, 1, 0, 0), mode='replicate')

    conf_dy = conf[:, :, :-1, :] - conf[:, :, 1:, :]
    conf_dy = F.pad(conf_dy, (0, 0, 0, 1), mode='replicate')

    # 拼接所有特征 [B, 6, H, W]
    enhanced_features = torch.cat([noise, conf, noise_dx, noise_dy, conf_dx, conf_dy], dim=1)
...
def parse_args(input_args=None):
    ...
    # Marigold-style training arguments
    parser.add_argument(
        "--use_enhanced_features",
        action="store_true",
        help="Whether to use enhanced features (gradient channels) for ControlNet conditioning.",
    )
    parser.add_argument(
        "--use_lora",
        action="store_true",
...
def main(args):
    ...
    # Log HuggingFace endpoint if using mirror
    if 'HF_ENDPOINT' in os.environ:
        logger.info(f"Using HuggingFace endpoint: {os.environ['HF_ENDPOINT']}")
    ...
    text_encoder = text_encoder_cls.from_pretrained(
        ...
        local_files_only=False,  # Allow online but use cache
    )
    ...
    if args.depthcad_path:
        ...
        # Marigold-style: Determine number of conditioning channels
        if args.use_enhanced_features:
            conditioning_channels = 6  # noise, conf, and their gradients
            logger.info(f"Initializing DepthCAD with {conditioning_channels} conditioning channels (enhanced features)")
        else:
            conditioning_channels = 2  # Original: noise and conf
            logger.info(f"Initializing DepthCAD with {conditioning_channels} conditioning channels (original)")

        depthcad = ControlNetModel.from_unet(unet, conditioning_channels=conditioning_channels)

    # Marigold-style: Apply LoRA for efficient fine-tuning
    if args.use_lora:
        if not PEFT_AVAILABLE:
            raise ValueError(
                "--use_lora flag is set but PEFT library is not installed. "
                "Please install it with: pip install peft"
            )
        logger.info("Applying LoRA to DepthCAD for parameter-efficient fine-tuning")
        ...
        depthcad = get_peft_model(depthcad, lora_config)
        depthcad.print_trainable_parameters()

        logger.info(f"LoRA enabled: rank={args.lora_rank}, alpha={args.lora_alpha}, dropout={args.lora_dropout}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(depthcad):
                ...
                # Marigold-style: Enhanced ControlNet conditioning
                if args.use_enhanced_features:
                    # Use enhanced features with gradients
                    depthcad_image = compute_enhanced_features(
                        batch["noises"].to(dtype=weight_dtype),
                        batch["confs"].to(dtype=weight_dtype)
                    )  # torch.Size([B, 6, H, W])
                else:
                    # Original simple concatenation
                    depthcad_image = torch.concatenate([batch["noises"], batch["confs"]], dim=1).to(dtype=weight_dtype)  # torch.Size([B, 2, H, W])
    ...
    if accelerator.is_main_process:
        ...
        # Marigold-style: Save LoRA adapters if enabled
        if args.use_lora:
            # Save LoRA adapters separately
            lora_output_dir = os.path.join(args.output_dir, "lora")
            os.makedirs(lora_output_dir, exist_ok=True)
            depthcad.save_pretrained(lora_output_dir)
            logger.info(f"LoRA adapters saved to {lora_output_dir}")

            # Also save the full merged model for inference
            if hasattr(depthcad, 'merge_and_unload'):
                merged_model = depthcad.merge_and_unload()
                merged_model.save_pretrained(args.output_dir)
                logger.info(f"Merged model saved to {args.output_dir}")
            else:
                # Fallback: just save the base model
                base_model = depthcad.get_base_model() if hasattr(depthcad, 'get_base_model') else depthcad
                base_model.save_pretrained(args.output_dir)
                logger.info(f"Base model saved to {args.output_dir}")
        else:
