"""
DepthCAD Training Script for PBRT Dataset

Usage:
    # Single GPU
    python train_pbrt.py --dataset_name pbrt_dataset --output_dir output_pbrt

    # Multi-GPU (using accelerate)
    accelerate launch train_pbrt.py --dataset_name pbrt_dataset --output_dir output_pbrt
"""
import argparse
import logging
import math
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default="default",
        help=(
            "The config name of the Dataset to train on. "
            "Available configs: 'default' (without mask), 'masked' (with amplitude mask)."
        ),
    )
    parser.add_argument(
        "--train_list_path",
        ...
        default=None,
        help=(
            "Path to a .txt file containing list of allowed paths to include in training. "
            "Each line should contain a relative path like 'bathroom/0'. "
            "If not provided, all data will be used."
        ),
    )
    parser.add_argument(
        "--tracker_project_name",
        type=str,
        default="train_depthcad_pbrt",
        help=(
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    # Filter dataset based on train.txt if provided
    if args.train_list_path:
        # Read allowed paths from train.txt
        with open(args.train_list_path, 'r') as f:
            allowed_paths = set()
            for line in f:
                line = line.strip()
                if line:
                    # Extract the relative path like "bathroom/0"
                    allowed_paths.add(line)

        # Filter dataset to only include samples from train.txt
        def filter_by_train_list(example):
            # Extract relative path from the full path
            # Example: /data/pre_student/GJ/DepthCAD/pbrt_dataset/data/ideal_IQ/bathroom/0/100_A.npy
            # -> bathroom/0
            ideal_path = Path(example['ideal_IQ_path'])
            # Path: .../ideal_IQ/bathroom/0/100_A.npy
            # Need: bathroom/0
            # parents[1] = ideal_IQ/bathroom, parents[2] = ideal_IQ
            scene_dir = ideal_path.parents[1].relative_to(ideal_path.parents[2])  # bathroom
            index_dir = ideal_path.parent.name  # 0
    ...
    def tokenize_captions(examples, is_train=True):
        """Tokenize captions. For PBRT, prompts are empty strings."""
        captions = []
        for caption in examples["prompt"]:
            if isinstance(caption, str):
                captions.append(caption if caption else "")  # Use empty string for PBRT
    ...
    def preprocess_train(examples):
        # print("Sample shapes:", 
        #     np.load(examples["ideal_IQ_path"][0]).shape,
        #     np.load(examples["noise_IQ_path"][0]).shape,
        #     np.load(examples["conf_path"][0]).shape)

        """Preprocess training data for PBRT dataset.

        PBRT data format:
        - ideal_IQ_path: path to ideal IQ .npy file (shape: 512x512 after preprocessing)
        - noise_IQ_path: path to noise IQ .npy file (shape: 512x512)
        - conf_path: path to confidence .npy file (shape: 512x512)
        - prompt: empty string for PBRT dataset
        """
        import cv2
        ...
        for ideal_IQ_path, noise_IQ_path, conf_path in zip(
            ...
            # Load ideal IQ (single channel, 2D array)
            ...
            # Load noise IQ (single channel, 2D array)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(depthcad):
                ...
                depthcad_image = torch.concatenate([batch["noises"], batch["confs"]], dim=1).to(dtype=weight_dtype)    # torch.Size([16, 2, 512, 512])
