...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    data_transforms = transforms.Compose(
        [
            ...
            transforms.Resize((args.resolution, args.resolution), antialias=None),
        ]
    )
...
def main(args):
    ...
    logger.info(f"正在加载 HYPIR LoRA 权重: ./HYPIR_sd2.pth")
    try:
        # 加载 HYPIR LoRA 权重
        from peft import LoraConfig, get_peft_model

        hypir_state_dict = torch.load("./HYPIR_sd2.pth", map_location="cpu")
        if "state_dict" in hypir_state_dict:
            hypir_state_dict = hypir_state_dict["state_dict"]

        # 创建 LoRA 配置（根据 HYPIR 的典型配置）
        # HYPIR 通常在 attention 层使用 LoRA
        lora_config = LoraConfig(
            r=256,  # HYPIR 使用的 rank
            lora_alpha=256,
        ...
        # 加载 LoRA 权重（直接加载到 UNet 的 LoRA 层中）
        load_result = unet.load_state_dict(hypir_state_dict, strict=False)
        logger.info(f"成功加载 HYPIR LoRA！缺失的键: {len(load_result.missing_keys)}, 意外的键: {len(load_result.unexpected_keys)}")

        # 融合并卸载 LoRA，将权重合并回原始 UNet 结构
        # 这样 ControlNetModel.from_unet 才能正常工作
        unet = unet.merge_and_unload()
        logger.info("HYPIR LoRA 权重已融合到 UNet 主干中。")
    except Exception as e:
        logger.warning(f"HYPIR LoRA 加载失败，继续训练 (不影响基础功能): {e}")
        import traceback
