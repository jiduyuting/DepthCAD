...
# Import from process.py (mask-enabled version) instead of preprocess.py
try:
    from pbrt_dataset.process import load_raw as load_raw_pbrt, compute_gradient_confidence
    PBRT_AVAILABLE = True
    print("Successfully imported from pbrt_dataset.process")
except ImportError as e:
    print(f"Failed to import from pbrt_dataset.process: {e}")
...
def inference(pipe, noise, conf, scale, target_size=(240, 320)):
    ...
    # 初始化用于存储推理结果的数组 (使用推理尺寸 256x256)
    pred_IQs_infer = np.zeros((6, infer_h, infer_w))

    for i in range(6):
        # 3. 预处理：将当前通道的 noise resize 到推理尺寸
        # 确保 noise[i] 和 conf_resized 都是 (256, 256)，这样 stack 就不会报错了
...
if __name__ == '__main__':
    ...
    if dataset_type == "pbrt":
        if not PBRT_AVAILABLE:
            raise ImportError("pbrt_dataset.process not available. Please check your imports.")
