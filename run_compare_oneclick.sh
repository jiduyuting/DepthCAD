...
usage() {
  cat <<'EOF'
Usage:
  bash run_compare_oneclick.sh [real|pbrt|all]
...
run_pbrt_compare() {
  ...
  if [[ "$PBRT_RUN_PROPAINTER" != "0" ]]; then
    ...
    "$CONTROL_PYTHON_BIN" export_pbrt_propainter_case.py \
      --cache_dir "$PBRT_CACHE_DIR" \
      --output_dir "$PBRT_PROPAINTER_CASE"

    echo "[pbrt] running ProPainter"
    "$CONTROL_PYTHON_BIN" run_external_inpainting_far_pic.py run-propainter \
    ...
    "$CONTROL_PYTHON_BIN" eval_pbrt_external_inpainting.py \
