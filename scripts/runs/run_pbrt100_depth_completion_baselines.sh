...
run_ldcm() {
  ...
  DEVICE="${DEVICE}" bash scripts/runs/run_ldcm_full_pbrt.sh
}

run_lingbot() {
  CACHE_ROOT="${CACHE_ROOT}" SPLIT_JSON="${SPLIT_JSON}" SPLIT="${SPLIT}" \
  OUTPUT_DIR="${ROOT_DIR}/output/pbrt100_depth_completion/lingbot_dc_zero_shot" \
  DEVICE="${DEVICE}" bash scripts/runs/run_lingbot_full_pbrt.sh
}
...
run_omnidc() {
  ...
  GPU="${GPU}" bash scripts/runs/run_omnidc_full_pbrt.sh
}
...
run_dmd3c() {
  ...
  GPU="${GPU}" bash scripts/runs/run_dmd3c_full_pbrt.sh
}
...
Examples:
  METHODS="rgbd_lfrd2" bash scripts/runs/run_pbrt100_depth_completion_baselines.sh
  METHODS="ldcm lingbot omnidc dmd3c depthor completionformer" GPU=0 bash scripts/runs/run_pbrt100_depth_completion_baselines.sh
  METHODS="all" bash scripts/runs/run_pbrt100_depth_completion_baselines.sh
