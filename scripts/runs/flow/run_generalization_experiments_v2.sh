...
[[ -d "${PBRT_CACHE}" && -f "${PBRT_LISTS}/train.txt" && -f "${BASE_CHECKPOINT}" ]] || {
  echo "Missing PBRT cache/lists/checkpoint" >&2; exit 2;
}
...
"${PYTHON_BIN}" scripts/flow/make_scene_holdout_flow_lists_v2.py \
...
train_scene scene_baseline
train_scene scene_mixed_mask --mask_augment --mask_augment_probability 0.5 \
...
eval_one scene_baseline_holdout "${RUN_ROOT}/scene_baseline/best_global.pt" "${PBRT_CACHE}" "${TEST_LIST}"
eval_one scene_mixed_mask_holdout "${RUN_ROOT}/scene_mixed_mask/best_global.pt" "${PBRT_CACHE}" "${TEST_LIST}"
eval_one full_pbrt_to_flat "${BASE_CHECKPOINT}" "${FLAT_CACHE}" "${FLAT_CACHE}/test.txt"
