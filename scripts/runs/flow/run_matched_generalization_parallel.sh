...
# Four-GPU matched-protocol pilot/full run.
# GPU0/1: contemporary-bathroom baseline/mixed-mask.
# GPU2/3: white-room baseline/mixed-mask.
...
MODE="${MODE:-pilot}"; EPOCHS="${EPOCHS:-5}"
RUN_ROOT="${RUN_ROOT:-${ROOT_DIR}/output/matched_generalization_parallel/$(date +%Y%m%d_%H%M%S)}"
...
REBUILD_FLAT_CACHE="${REBUILD_FLAT_CACHE:-0}"
if [[ ! -f "${FLAT_CACHE}/manifest.json" || "${REBUILD_FLAT_CACHE}" == "1" ]]; then
  "${PYTHON_BIN}" scripts/flow/prepare_flat_matched_flow_cache.py --flat_data_root "${FLAT_DATA}" --depth_root "${FLAT_DEPTH}" --output_root "${FLAT_CACHE}" --overwrite >"${RUN_ROOT}/logs/prepare_flat_matched.log" 2>&1
fi
...
eval_one "${GPU0}" full_pbrt_to_flat_matched "${BASE_CHECKPOINT}" "${FLAT_CACHE}/test.txt" "${FLAT_CACHE}" || status=1
printf '%s\n' "mode=${MODE}" "epochs=${EPOCHS}" "pbrt_cache=${PBRT_CACHE}" "flat_cache=${FLAT_CACHE}" "base_checkpoint=${BASE_CHECKPOINT}" "FLAT targets are paired pseudo-GT; masks match PBRT amplitude-driven protocol." >"${RUN_ROOT}/README.txt"
