...
try:
    from pbrt_dataset.process_mask import load_raw as load_raw_pbrt, compute_gradient_confidence
    PBRT_AVAILABLE = True
except ImportError:
    PBRT_AVAILABLE = False
try:
...
def inference(pipe, noise, conf, scale, target_size=(240, 320)):
    ...
    内部强制使用 256x256 进行推理，最后还原回 target_size
    """
    # 1. 定义模型推理需要的尺寸 (Stable Diffusion 需要是 64 的倍数)
    # 你的原始代码中为了方便操作提到了 256x256，这里我们显式定义它
...
if __name__ == '__main__':
    ...
    if dataset_type == "pbrt":
        ...
        noise = load_raw_pbrt(noise_file, target_size=target_size, sqrt_in=True)
        ...
        noise = load_raw_flat(noise_file)
        print(f"Loaded noise IQ shape: {noise.shape}")
