...
if __name__ == '__main__':
    ...
    # Use DepthEstimatorTorchArgmax (GPU accelerated, same algorithm as IQ_to_depth)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    estimator = DepthEstimatorTorch(device=device)
    pred_IQs_tensor = torch.from_numpy(pred_IQs).unsqueeze(0).to(device)  # (1, 6, H, W)
    depth_tensor = estimator.process(pred_IQs_tensor)
    depth = depth_tensor.squeeze(0).cpu().numpy()  # (H, W)
